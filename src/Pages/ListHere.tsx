...
import { defaultOfferingList } from '../Utils/DefaultOfferingList';
import { timeOptions } from '../Utils/Utils';
import { useDispatch, useSelector } from 'react-redux';
import { addBusiness, addBusinessLoading, categories as categoriesList } from '../Redux/selectors';
...
const ListHere: React.FC<BusinessFormProps> = () => {
    ...
    const [defaultOfferings, setDefaultOfferings] = useState<{
        _id: string;
        name: string;
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        dispatch(Actions.business.business.addBusinessActions.request({
            businessData: {
                ...formData,
                photoUrls: imageUrls,
            }
    ...
    const validateForm = () => {
        ...
        //     showAlert('Please enter a valid website', 'error');
        //     return false;
        // }
    ...
    const getTextField = (textFieldInfo: listTextFieldType) => {
        const { gridSize, name, display, labelName, value, onChange, isRequired, isMultiline } = textFieldInfo;
        const adornment = <Typography variant="body1" sx={{ color:'text.disabled', marginRight: 1 }}>{display ? display : ''}</Typography>;
        return (
            <Grid size={gridSize}>
            <TextField
                required={isRequired}
    ...
    const getDropdownField = (dropdownFieldInfo: listSelectFieldType) => {
        ...
        return (<Grid size={gridSize}>
            <Tooltip title={optionsList.find(o => o._id === value)?.name || value}>
                <TextField
                    required={isRequired}
                    select={selectValue}
                    label={t(labelName)}
    ...
    const buildOfferGrid = (index: number) => {
        ...
        return (<Paper elevation={3} sx={{ padding: '1em', marginBottom: '1em' }}>
            <Grid container spacing={2}>
                ...
                {getTextField({
                    display: currencyList.find((currency) => currency._id === formData.offerings[index]?.price?.currency)?.symbol || '$',
    ...
    const buildOfferingsAccordion = () => {
        return <AccordionDetails id="offerings">
            {/* {
                <Grid container sx={{ marginBottom: 3, display: 'flex', alignItems: 'center'}}>
                    <Grid size={{ xs: 3 }}>
                        <Typography variant="subtitle1" sx={{ fontWeight: 'bold' }}>
                            {t('useDefaultOfferings')}
                        </Typography>
                    </Grid>
                    {getDropdownField(
                        {
                            gridSize: { xs: 6 },
                            labelName: 'defaultOfferings',
                            value: defaultOfferings?._id || '',
                            optionsList: defaultOfferingList,
                            isRequired: false,
                            selectValue: true,
                            onChange: (e: React.ChangeEvent<{ value: unknown }>) => {
                                const defaultOffering = defaultOfferingList.find((offering) => offering._id === e.target.value);
                                setDefaultOfferings(defaultOffering);
                            } 
                        }
                    )}
                    <Grid size={{ xs: 3 }} sx={{ display: 'flex', justifyContent: 'center' }}>
                        <Button variant="outlined" onClick={
                            () => {
                                if (!defaultOfferings) {
                                    showAlert('Please select a default offering list', 'error');
                                    return;
                                }
                                if (defaultOfferings?.offerings) {
                                    formData.offerings = [...defaultOfferings.offerings, ...formData.offerings];
                                }
                                setFormData({ ...formData });
                            }
                        }>
        ...
        return (
            <AccordionDetails>
                <Grid container spacing={2}>
                    ...
                    {getDropdownField(
                        {
                            gridSize: { xs: formData.address?.isVirtualOnly ? 12 : 6, lg: 6 },
                            labelName: 'country',
                    ...
                    {getDropdownField(
                        {
                            ...
                            optionsList: countryList.find((country) => country._id === formData.address?.country)?.provinces || [],
                            isRequired: true,
                            selectValue: true,
                            onChange: (e: React.ChangeEvent<{ value: unknown }>) => {
                                setFormData({
                    ...
                    {getTextField({ gridSize: { xs: 6 }, labelName: 'postalCode', value: formData.address?.postalCode, onChange: handleAddressChange, isRequired: true })}
                </Grid>
            </AccordionDetails>
        );
    };
    ...
    const initialTextFields = [
        { gridSize: { xs: 12 }, name: "name", labelName: 'businessName', value: formData.name, onChange: handleChange, isRequired: true },
        { gridSize: { xs: 12 }, labelName: 'email', value: formData.email, onChange: handleChange, isRequired: true },
        { gridSize: { xs: 12 }, labelName: 'phone', value: formData.phone, onChange: handleChange, isRequired: true },
        { gridSize: { xs: 12 }, labelName: 'website', value: formData.website, onChange: handleChange, isRequired: false }
    ];

    const addressFields = [
        { gridSize: { xs: 12 }, labelName: 'street', value: formData.address?.street, onChange: handleAddressChange, isRequired: false },
    ...
    const accordions = [
        // { title: 'listingUser', fields: listingUserTextFields, isOptional: false },
    ...
    return (
        ...
        <Container maxWidth="md" sx={{ marginTop: '5em' }}>
            ...
            <form onSubmit={handleSubmit} id={'listHereForm'}>
                <Grid container spacing={2}>
                    <Grid size={{ xs: 6 }}>
